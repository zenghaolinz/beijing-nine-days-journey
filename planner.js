...
const ORDER_PAIRS=[['ditan','lama'],['lama','guozijian'],['square','palace']];
...
for(const b of reservations){const error=validate(b,start);if(error){issues.push((POIS[b.poi]?.name||'预约')+'：'+error);continue;}if(booked.has(b.poi)){issues.push(POIS[b.poi].name+'有重复预约，请保留一个。');continue;}booked.add(b.poi);const p=POIS[b.poi],day=dateNumber(b.date)-dateNumber(start),time=minutes(b.time);days[day].push({...p,id:b.poi,locked:true,start:time,end:time+b.duration,duration:b.duration,date:b.date});if(day===0&&time<900)warnings.push(p.name+'在抵达日下午前，请确认到京时间。');if(time<p.earliest||time+b.duration>p.latest)warnings.push(`${p.name}的${clock(time)}入场或计划结束时间超出网页预设游览窗口，请核对票面、开放时段和时长。`);if(b.poi==='palace'&&new Date(b.date+'T12:00:00Z').getUTCDay()===1)warnings.push('故宫预约在周一：一般闭馆，法定节假日例外，请核对票面与官方公告。');}
days.forEach((items,day)=>{items.sort((a,b)=>a.start-b.start);for(let i=1;i<items.length;i++){const a=items[i-1],b=items[i],need=travel(a,b);if(a.end+need>b.start)issues.push(`第${day+1}天：${a.name} ${clock(a.start)}–${clock(a.end)}，到${b.name}还需预留约${need}分钟，赶不上${clock(b.start)}的预约。`);}for(const [before,after] of ORDER_PAIRS){const first=items.findIndex(x=>x.id===before),second=items.findIndex(x=>x.id===after);if(first>=0&&second>=0&&first>second)warnings.push(`第${day+1}天的预约是${POIS[after].name}先于${POIS[before].name}；已按票面时刻保留，请核对当天进出路线。`);}});
for(const id of ['pku','tsinghua'])if(!booked.has(id))unplaced.push({...POIS[id],id,reason:'尚未记录有效入校预约。预约成功后填写日期和入场时间，网页再将其排入行程。'});
const all=Object.entries(POIS).filter(([id,p])=>!booked.has(id)&&!p.conditional&&!(final==='bird'&&id==='bridge')&&!(final==='bridge'&&id==='bird')).map(([id,p])=>({...p,id,locked:false}));
// Keep the traveler's requested neighborhood and entrance order when times are flexible.
const withinGroup={ditan:0,lama:1,guozijian:2,square:0,palace:1};
all.sort((a,b)=>a.group-b.group||(withinGroup[a.id]??0)-(withinGroup[b.id]??0)||(a.latest-a.earliest-a.duration)-(b.latest-b.earliest-b.duration));
...
let best=null;for(let i=0;i<=items.length;i++){if(ORDER_PAIRS.some(([before,after])=>p.id===before&&items.slice(0,i).some(x=>x.id===after)||p.id===after&&items.slice(i).some(x=>x.id===before)))continue;const prev=items[i-1],next=items[i],from=prev||home,transit=travel(from,p),floor=day===0?900:480;let st=Math.max(p.earliest,prev?prev.end+transit:floor+transit); // room for a seated lunch between visits
...
for(const p of all){let best=null;for(let day=1;day<9;day++){const c=candidate(p,day);if(c&&(!best||c.score<best.score))best=c;}if(!best){unplaced.push({...p,reason:'在保守游览时段内没有足够空档，未强行塞入。'});continue;}days[best.day].push({...p,...best});days[best.day].sort((a,b)=>a.start-b.start);}
